...
if len(sys.argv) < 3:
	print("You must pass the results main folder path and the player deck name as an argument to the script!")
	exit()
else:
...
for dir1 in os.listdir(path):
	if os.path.isdir(path + "/" + dir1):
		results[dir1] = {}
		for dir2 in os.listdir(path + "/" + dir1 + "/" + player_deck):
			if os.path.isdir(path + "/" + dir1 + "/" + player_deck + "/" + dir2):
				results[dir1][dir2] = {"numGames": 0, "numWins": 0, "cardData": {}}
				for file in os.listdir(path + "/" + dir1 + "/" + player_deck + "/" + dir2):
					if os.path.isfile(path + "/" + dir1 + "/" + player_deck + "/" + dir2 + "/" + file):
						entry = results[dir1][dir2]
					
						file_data = open(path + "/" + dir1 + "/" + player_deck + "/" + dir2 + "/" + file, 'r')
