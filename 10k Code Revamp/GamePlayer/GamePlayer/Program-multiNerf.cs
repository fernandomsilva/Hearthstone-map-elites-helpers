...
namespace GamePlayer
{
    internal class Program
    {
		...
		private static void NerfCards(List<string[]> nerfs)//string nerf_filepath)
		{
		...
		private static void NerfOneCard(string[] nerf)
		{
        ...
        private static void Main(string[] args)
        {
			...
			List<string[]> nerfs = new List<string[]>();
			List<string[]> default_card_data = new List<string[]>();
			...
			numGames = 2;
			stepSize = 0;
			player_decks_file = "paladin_2_control_deck.csv";
			...
			string[] file_data = System.IO.File.ReadAllLines("nerfs.csv");
			...
			file_data = System.IO.File.ReadAllLines("default_card_stats.csv");
			
			for (int i=0; i<file_data.Length; i++)
			{
				string textLine = file_data[i];
				string[] nerf_data = textLine.Split2(';');
				default_card_data.Add(nerf_data);
			}
					...
					for (int y=0; y<opponents.Count; y++)
					{
						...
						if (player[0] != opponent[0] && !previous_matchups.Contains("|" + player[0] + "X" + opponent[0] + "|"))
						{
							...
							while (j < number_of_loops)
							{
								...
								string overallGameStat = folderName + "/" + nerf[0] + "/" + player[0] + "/" + opponent[0];
