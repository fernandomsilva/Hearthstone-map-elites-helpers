	...
	if os.path.isdir(folder + "/gen" + str(i)):
		...
		if os.path.isfile(filepath):
			...
			for line in file_data:
				...
				if '[' in temp[0]:
					euclideanValue = float(temp[0].split('[')[1])
					weightValue = float(temp[1].split(']')[0])
				else:
					euclideanValue = float(temp[0].split('(')[1])
					weightValue = float(temp[1])
				
				results.append([vector, euclideanValue, i, weightValue])#calculateVectorWeight(vector, default_card_stats_list)])
...
for entry in results:
	output_file.write(str(entry[0]) + ';' + str(entry[1]) + ';' + str(entry[3]) + ';' + str(entry[2]) + '\n')
output_file.close()
