...
namespace GamePlayer
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
			...
			while (true)
			{
				while (!File.Exists(folderName + "/gen" + g + "/input.txt"))
				{
					Thread.Sleep(1000);
				}
        ...
        public static List<List<int>> getIndividualsFromFile(string path)
        {
			...
			{
				string textLines = file_data[i];
				List<int> new_entry = new List<int>();
				
				string list_string = (textLines.Split2('[')[1]).Split2(']')[0];
				string[] values = list_string.Split2(',');
				
				for (int k=0; k<values.Length; k++)
				{
					new_entry.Add(int.Parse(values[k]));
				}
